...
def test_empty_items():
    with pytest.raises(ValueError):
        process_checkout({"user_id": 1, "items": [], "currency": "USD"})

def test_missing_user_id():
    with pytest.raises(ValueError):
        process_checkout({"items": [{"price": 10, "qty": 1}], "currency": "USD"})

def test_invalid_item():
    with pytest.raises(ValueError):
        process_checkout({"user_id": 1, "items": [{"qty": 1}], "currency": "USD"})

def test_zero_price():
    with pytest.raises(ValueError):
        process_checkout({"user_id": 1, "items": [{"price": 0, "qty": 1}], "currency": "USD"})
